 ...
 * scene graph in action, and a tinted glass pane in front to show
 * transparency.
 */
function buildScene3D(checker) {
  ...
  sun.shadow.camera.size = 7;
  sun.shadow.camera.near = 0.1;
  sun.shadow.camera.far = 30;
  sun.shadow.camera.lookAt(0, 0.5, 0);
  scene.add(sun);
  ...
  // The satellite doubles as a lamp: a PointLight parented to it, so
  // its warm glow orbits the cube (and follows when the cube is
  // dragged) purely through the scene graph.
  satellite.add(new PointLight([1, 0.75, 0.35], 1.5));

  // A wireframe globe hovering over the scene: WireframeGeometry turns
  // any triangle geometry into its unique edges, drawn with the
  // line-list pipeline. Kept out of the draggable list — picking
  // traverses children, so overlaying it on the sphere would steal
  // the sphere's drag.
  const globe = new Mesh(
    new WireframeGeometry(new SphereGeometry(0.5, 16, 10)),
    new BasicMaterial({ color: [0.4, 0.85, 1], topology: 'line-list' }),
  );
  globe.position.set(0.3, 2.3, -0.6);
  scene.add(globe);

  // A tinted glass pane: `transparent` draws it after the opaque
  // meshes with alpha blending, so the scene shows through. Culling is
  // off so it is also visible from behind when orbiting.
  const glass = new Mesh(
    new BoxGeometry(2.4, 1.4, 0.06),
    new LambertMaterial({
      color: [0.55, 0.8, 1, 0.35],
      transparent: true,
      cullMode: 'none',
    }),
  );
  glass.position.set(0.3, 0.8, 1.7);
  glass.rotation.y = -0.25;
  glass.receiveShadow = true;
  scene.add(glass);

  return { scene, cube, sphere, pivot, satellite, glass, globe };
}
