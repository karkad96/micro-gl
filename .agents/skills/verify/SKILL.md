...
- 3D boot shows `6 objects` (ground, cube, sphere, satellite — which
  carries an orbiting PointLight — a transparent glass pane, and a
  wireframe globe overhead); 2D shows `4 objects` (card, disc, overlay,
  satellite).
- Stress button cycles 0 → 500 → 2,000 → 8,000 → off; counts are N + 6
  in 3D and N + 4 in 2D.
